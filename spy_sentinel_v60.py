...
print("\nV5.2 GRADIENT-BOOSTING DEVELOPMENT")

from sklearn.ensemble import HistGradientBoostingClassifier

gb_fold_results = []
gb_confidence_results = []
...
gb_mean_accuracy = (
    gb_fold_df["accuracy"].mean()
)

gb_worst_accuracy = (
    gb_fold_df["accuracy"].min()
)

gb_best_accuracy = (
    gb_fold_df["accuracy"].max()
)

print("\nGRADIENT-BOOSTING STABILITY")
print(f"Mean accuracy: {gb_mean_accuracy:.2f}%")
print(f"Worst fold:    {gb_worst_accuracy:.2f}%")
print(f"Best fold:     {gb_best_accuracy:.2f}%")

print("\nGB HIGH-CONFIDENCE TEST")
...
for train_idx, test_idx in tscv.split(X):
    ...
    probabilities = gb_model.predict_proba(
        X_test
    )[:, 1]
    ...
    gb_confidence_results.append({
        "signals": len(selected_probs),
        "accuracy": selected_accuracy,
    })

gb_conf_df = pd.DataFrame(
    gb_confidence_results
)

if not gb_conf_df.empty:

    gb_hc_signals = int(
        gb_conf_df["signals"].sum()
    )

    gb_hc_accuracy = np.average(
        gb_conf_df["accuracy"],
        weights=gb_conf_df["signals"]
    )
...
print(
    f"Logistic HC accuracy: "
    f"{hc_accuracy:.2f}%"
)

print(
    f"Random Forest HC accuracy: "
    f"{rf_hc_accuracy:.2f}%"
)

print(
    f"Gradient Boosting HC accuracy: "
    f"{gb_hc_accuracy:.2f}%"
)

print("\nV5.2 GB DEVELOPMENT GATE")

gb_candidate = (
    gb_mean_accuracy >= 52
    and gb_worst_accuracy >= 48
    and gb_hc_accuracy >= 55
    and gb_hc_signals >= 100
)

if gb_candidate:
    gb_verdict = (
        "GB CANDIDATE READY FOR INDEPENDENT TEST"
    )
else:
    gb_verdict = (
        "GB MODEL NOT STRONG ENOUGH"
    )

print(f"Verdict: {gb_verdict}")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

gb_fold_df.to_csv(
    "spy_sentinel_gb_cv_results.csv",
    index=False
)
...
pd.DataFrame([{
    ...
    "high_confidence_signals": gb_hc_signals,
    "high_confidence_accuracy": round(
        gb_hc_accuracy, 3
    ),
    "candidate": gb_candidate,
    "verdict": gb_verdict,
}]).to_csv(
    "spy_sentinel_gb_summary.csv",
    index=False
)

print("Gradient-Boosting development saved")

print("\nV5.3 MEANINGFUL-MOVE ML DEVELOPMENT")

move_ml = ml.copy()

MOVE_THRESHOLD = 0.001

move_ml["move_class"] = np.where(
    move_ml["future_return_60m"] >= MOVE_THRESHOLD,
    1,
    np.where(
        move_ml["future_return_60m"] <= -MOVE_THRESHOLD,
        0,
...
move_data = move_ml.dropna(
    subset=ML_FEATURES + ["move_class"]
).copy()

move_data["move_class"] = (
    move_data["move_class"].astype(int)
)

print(f"Move threshold: {MOVE_THRESHOLD * 100:.2f}%")
print(f"Meaningful-move rows: {len(move_data)}")

X_move = move_data[ML_FEATURES]
y_move = move_data["move_class"]

up_moves = int((y_move == 1).sum())
down_moves = int((y_move == 0).sum())

print("\nMEANINGFUL-MOVE BALANCE")
print(f"Up moves:   {up_moves}")
print(f"Down moves: {down_moves}")

print("\nMEANINGFUL-MOVE TIME-SERIES CV")

move_fold_results = []

move_tscv = TimeSeriesSplit(
    n_splits=5
)

for fold_number, (train_idx, test_idx) in enumerate(
    move_tscv.split(X_move),
    start=1
):

    X_train = X_move.iloc[train_idx]
    X_test = X_move.iloc[test_idx]
    ...
    move_fold_results.append({
        "fold": fold_number,
        "signals": len(test_idx),
        "accuracy": acc,
    })
...
move_fold_df = pd.DataFrame(
    move_fold_results
)

move_mean_accuracy = (
    move_fold_df["accuracy"].mean()
)

move_worst_accuracy = (
    move_fold_df["accuracy"].min()
)

move_best_accuracy = (
    move_fold_df["accuracy"].max()
)

print("\nMEANINGFUL-MOVE STABILITY")
print(f"Mean accuracy: {move_mean_accuracy:.2f}%")
print(f"Worst fold:    {move_worst_accuracy:.2f}%")
print(f"Best fold:     {move_best_accuracy:.2f}%")

print("\nMEANINGFUL-MOVE HIGH-CONFIDENCE TEST")

move_hc_results = []

for train_idx, test_idx in move_tscv.split(X_move):
    ...
    accuracy = (
        selected_pred == selected_actual
    ).mean() * 100

    move_hc_results.append({
        "signals": len(selected_prob),
        "accuracy": accuracy,
    })

move_hc_df = pd.DataFrame(
    move_hc_results
)

if not move_hc_df.empty:

    move_hc_signals = int(
        move_hc_df["signals"].sum()
    )

    move_hc_accuracy = np.average(
        move_hc_df["accuracy"],
        weights=move_hc_df["signals"]
    )

else:

    move_hc_signals = 0
    move_hc_accuracy = 0.0

print(f"High-confidence signals: {move_hc_signals}")
print(
    f"High-confidence accuracy: "
    f"{move_hc_accuracy:.2f}%"
)

print("\nV5.3 DEVELOPMENT GATE")

move_candidate = (
    move_mean_accuracy >= 52
    and move_worst_accuracy >= 48
    and move_hc_accuracy >= 55
    and move_hc_signals >= 100
)

if move_candidate:
    move_verdict = (
        "MEANINGFUL-MOVE MODEL READY FOR INDEPENDENT TEST"
    )
else:
    move_verdict = (
...
print(f"Verdict: {move_verdict}")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

move_fold_df.to_csv(
    "spy_sentinel_meaningful_move_cv.csv",
    index=False
)

pd.DataFrame([{
    "threshold_pct": MOVE_THRESHOLD * 100,
    "rows": len(move_data),
    "mean_accuracy": round(move_mean_accuracy, 3),
    "worst_accuracy": round(move_worst_accuracy, 3),
    "best_accuracy": round(move_best_accuracy, 3),
    "high_confidence_signals": move_hc_signals,
    "high_confidence_accuracy": round(move_hc_accuracy, 3),
    "candidate": move_candidate,
    "verdict": move_verdict,
}]).to_csv(
    "spy_sentinel_meaningful_move_summary.csv",
    index=False
)

print("Meaningful-move research saved")

print("\nV5.4 ML DATA SANITATION")

ml_clean = ml_data.copy()

ml_clean[ML_FEATURES] = (
    ml_clean[ML_FEATURES]
...
X_clean = ml_clean[ML_FEATURES]
y_clean = ml_clean["target"].astype(int)

print(f"Clean ML rows: {len(ml_clean)}")
print(
    f"Finite features: "
    f"{np.isfinite(X_clean.to_numpy()).all()}"
)

print("\nV5.4 EXTRA-TREES DEVELOPMENT")

from sklearn.ensemble import ExtraTreesClassifier

et_tscv = TimeSeriesSplit(n_splits=5)
...
et_fold_df = pd.DataFrame(
    et_fold_results
)

et_mean_accuracy = (
    et_fold_df["accuracy"].mean()
)

et_worst_accuracy = (
    et_fold_df["accuracy"].min()
)

et_best_accuracy = (
    et_fold_df["accuracy"].max()
)

print("\nEXTRA-TREES STABILITY")
print(f"Mean accuracy: {et_mean_accuracy:.2f}%")
print(f"Worst fold:    {et_worst_accuracy:.2f}%")
print(f"Best fold:     {et_best_accuracy:.2f}%")

print("\nEXTRA-TREES HIGH-CONFIDENCE TEST")

for train_idx, test_idx in et_tscv.split(X_clean):
    ...
    selected_actual = y_test.to_numpy()[selected]
    ...
    et_hc_results.append({
        "signals": len(selected_prob),
        "accuracy": selected_accuracy,
    })

et_hc_df = pd.DataFrame(et_hc_results)

if not et_hc_df.empty:

    et_hc_signals = int(
        et_hc_df["signals"].sum()
    )

    et_hc_accuracy = np.average(
        et_hc_df["accuracy"],
        weights=et_hc_df["signals"]
    )
...
print(
    et_importance.head(10).to_string(
        index=False
    )
)

print("\nV5.4 EXTRA-TREES DEVELOPMENT GATE")

et_candidate = (
    et_mean_accuracy >= 52
    and et_worst_accuracy >= 48
    and et_hc_accuracy >= 55
    and et_hc_signals >= 100
)

if et_candidate:
    et_verdict = (
        "EXTRA-TREES CANDIDATE READY FOR INDEPENDENT TEST"
    )
else:
    et_verdict = (
        "EXTRA-TREES MODEL NOT STRONG ENOUGH"
    )

print(f"Verdict: {et_verdict}")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

et_fold_df.to_csv(
    "spy_sentinel_extra_trees_cv.csv",
    index=False
)

et_importance.to_csv(
    "spy_sentinel_extra_trees_importance.csv",
    index=False
)

print("Extra-Trees research saved")

print("\nV5.5 MULTI-HORIZON ML TARGET TEST")

ML_HORIZONS = {
    "15m": "future_return_15m",
    "30m": "future_return_30m",
    "60m": "future_return_60m",
    "90m": "future_return_90m",
}

multi_ml_results = []

print("Testing multiple horizons with the same feature set")

for label, return_col in ML_HORIZONS.items():

    horizon_data = ml.copy()

    horizon_data[ML_FEATURES] = (
        horizon_data[ML_FEATURES]
        .replace([np.inf, -np.inf], np.nan)
    )

    horizon_data = horizon_data.dropna(
        subset=ML_FEATURES + [return_col]
    ).copy()

    horizon_data["target_h"] = np.where(
        horizon_data[return_col] > 0,
        1,
        0
    )

    X_h = horizon_data[ML_FEATURES]
    y_h = horizon_data["target_h"]

    print(
        f"{label} rows: {len(horizon_data)}"
    )

    fold_acc = []

    tscv_h = TimeSeriesSplit(
        n_splits=5
    )

    for train_idx, test_idx in tscv_h.split(X_h):
    ...
    worst_acc = (
        float(np.min(fold_acc))
        if fold_acc
        else 0.0
    )

    best_acc = (
        float(np.max(fold_acc))
        if fold_acc
        else 0.0
    )

    multi_ml_results.append({
        "horizon": label,
    ...
    print(
        f"{label}: "
        f"mean {mean_acc:.2f}% | "
        f"worst {worst_acc:.2f}% | "
        f"best {best_acc:.2f}%"
    )

multi_ml_df = pd.DataFrame(
    multi_ml_results
)

print("\nMULTI-HORIZON ML COMPARISON")

print(
    multi_ml_df.to_string(
        index=False
    )
)

best_horizon_ml = multi_ml_df.sort_values(
...
print(
    f"Horizon: "
    f"{best_horizon_ml['horizon']}"
)

print(
    f"Mean accuracy: "
    f"{best_horizon_ml['mean_accuracy']:.2f}%"
)

print(
    f"Worst fold: "
    f"{best_horizon_ml['worst_accuracy']:.2f}%"
)

print("\nV5.5 HORIZON GATE")

horizon_candidate = (
    best_horizon_ml["mean_accuracy"] >= 52
    and best_horizon_ml["worst_accuracy"] >= 48
)

if horizon_candidate:
    horizon_verdict = (
        "ONE HORIZON WORTH DEEPER TESTING"
    )
else:
    horizon_verdict = (
        "NO ML HORIZON STRONG ENOUGH"
    )

print(f"Verdict: {horizon_verdict}")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

multi_ml_df.to_csv(
    "spy_sentinel_multi_horizon_ml.csv",
    index=False
)

pd.DataFrame([{
    "best_horizon": best_horizon_ml["horizon"],
    "mean_accuracy": round(
        best_horizon_ml["mean_accuracy"], 3
    ),
    "worst_accuracy": round(
        best_horizon_ml["worst_accuracy"], 3
    ),
    "candidate": horizon_candidate,
    "verdict": horizon_verdict,
}]).to_csv(
    "spy_sentinel_multi_horizon_ml_summary.csv",
    index=False
)

print("Multi-horizon ML research saved")

print("\nV5.6 ML BASELINE AUDIT")
...
print("Comparing ML against training-set majority baseline")

def audit_horizon(frame, return_col, horizon_name):
    ...
    X_audit = data_h[ML_FEATURES]
    y_audit = data_h["audit_target"]
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        fold_rows.append({
            ...
            "excess_accuracy": model_acc - baseline_acc,
            "auc": auc,
        })
...
for horizon_name, return_col in ML_HORIZONS.items():

    baseline_audit_rows += audit_horizon(
        ml,
        return_col,
        horizon_name
    )
...
print(
    f"Audit folds completed: "
    f"{len(baseline_audit_df)}"
)
...
audit_summary = (
    ...
    .agg(
        ...
        mean_excess_accuracy=(
            "excess_accuracy", "mean"
        ),
        worst_excess_accuracy=(
            "excess_accuracy", "min"
        ),
        mean_auc=(
            "auc", "mean"
...
best_adjusted = audit_summary.sort_values(
    [
        "mean_excess_accuracy",
        "mean_balanced_accuracy",
    ],
...
print("\nBEST BASELINE-ADJUSTED HORIZON")

print(
    f"Horizon: "
    f"{best_adjusted['horizon']}"
)

print(
    f"Model accuracy: "
    f"{best_adjusted['mean_model_accuracy']:.2f}%"
)

print(
    f"Majority baseline: "
    f"{best_adjusted['mean_baseline_accuracy']:.2f}%"
)

print(
    f"Excess accuracy: "
    f"{best_adjusted['mean_excess_accuracy']:+.2f}%"
)

print(
    f"Balanced accuracy: "
    f"{best_adjusted['mean_balanced_accuracy']:.2f}%"
)

print(
    f"Mean AUC: "
    f"{best_adjusted['mean_auc']:.2f}%"
)

print("\nV5.6 LEARNING-SIGNAL GATE")

real_learning_signal = (
    best_adjusted["mean_excess_accuracy"] >= 2.0
    and best_adjusted["mean_balanced_accuracy"] >= 52.0
    and best_adjusted["mean_auc"] >= 52.0
)

if real_learning_signal:
    learning_verdict = (
        "MODEL SHOWS A SMALL DEVELOPMENT LEARNING SIGNAL"
    )
else:
    learning_verdict = (
        "MODEL DOES NOT BEAT SIMPLE BASELINE RELIABLY"
    )

print(f"Verdict: {learning_verdict}")

print("THIS IS DEVELOPMENT ANALYSIS ONLY")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

baseline_audit_df.to_csv(
    "spy_sentinel_ml_baseline_folds.csv",
    index=False
)

audit_summary.to_csv(
    "spy_sentinel_ml_baseline_summary.csv",
    index=False
)

pd.DataFrame([{
    "best_horizon": best_adjusted["horizon"],
    "model_accuracy": round(
        best_adjusted["mean_model_accuracy"], 3
    ),
    "baseline_accuracy": round(
        best_adjusted["mean_baseline_accuracy"], 3
    ),
    "excess_accuracy": round(
        best_adjusted["mean_excess_accuracy"], 3
    ),
    "balanced_accuracy": round(
        best_adjusted["mean_balanced_accuracy"], 3
    ),
    "auc": round(
        best_adjusted["mean_auc"], 3
    ),
    "learning_signal": real_learning_signal,
    "verdict": learning_verdict,
}]).to_csv(
    "spy_sentinel_ml_learning_signal.csv",
    index=False
)

print("ML baseline audit saved")

print("\nV5.7 RETURN-REGRESSION DEVELOPMENT")
...
X_reg = reg[ML_FEATURES]
y_reg = reg["future_return_60m"]

print(f"Regression rows: {len(reg)}")

print("\nREGRESSION TIME-SERIES CV")

reg_tscv = TimeSeriesSplit(n_splits=5)

reg_fold_results = []
reg_predictions = []

for fold_number, (train_idx, test_idx) in enumerate(
    reg_tscv.split(X_reg),
    start=1
):
    ...
    for p, actual in zip(pred, y_test.to_numpy()):
...
mean_corr = reg_fold_df["correlation"].mean()
worst_corr = reg_fold_df["correlation"].min()
mean_mae = reg_fold_df["mae"].mean()

print("\nREGRESSION STABILITY")

print(f"Mean correlation: {mean_corr:.3f}")
print(f"Worst correlation:{worst_corr:.3f}")
print(f"Mean MAE:         {mean_mae * 100:.4f}%")

print("\nPREDICTED-MOVE SIGNAL TEST")

PRED_THRESHOLD = 0.001

reg_pred_df["signal"] = np.where(
    reg_pred_df["predicted_return"] >= PRED_THRESHOLD,
    "BULLISH",
    np.where(
        reg_pred_df["predicted_return"] <= -PRED_THRESHOLD,
        "BEARISH",
...
reg_signals = reg_pred_df[
    reg_pred_df["signal"].isin(
        ["BULLISH", "BEARISH"]
    )
].copy()

print(f"Regression trade signals: {len(reg_signals)}")

reg_signal_correct = (
    (
        (reg_signals["signal"] == "BULLISH")
        & (reg_signals["actual_return"] > 0)
    )
    |
    (
        (reg_signals["signal"] == "BEARISH")
        & (reg_signals["actual_return"] < 0)
    )
)

reg_signal_accuracy = (
    reg_signal_correct.mean() * 100
    if len(reg_signals)
    else 0.0
)

print("\nREGRESSION SIGNAL ACCURACY")
print(f"Accuracy: {reg_signal_accuracy:.2f}%")
print(f"Signals:  {len(reg_signals)}")
print(f"Edge:     {reg_signal_accuracy - 50:+.2f}%")

reg_signed_returns = np.where(
    reg_signals["signal"] == "BULLISH",
    reg_signals["actual_return"],
    -reg_signals["actual_return"]
)

reg_avg_return = (
    float(np.mean(reg_signed_returns)) * 100
    if len(reg_signed_returns)
    else 0.0
)

reg_median_return = (
    float(np.median(reg_signed_returns)) * 100
    if len(reg_signed_returns)
    else 0.0
)

print("\nREGRESSION RETURN QUALITY")
print(f"Average directional return: {reg_avg_return:+.4f}%")
print(f"Median directional return:  {reg_median_return:+.4f}%")

print("\nV5.7 REGRESSION GATE")

reg_candidate = (
    mean_corr >= 0.05
    and worst_corr > -0.10
    and reg_signal_accuracy >= 55
    and len(reg_signals) >= 100
    and reg_avg_return > 0
)

if reg_candidate:
    reg_verdict = (
        "REGRESSION CANDIDATE READY FOR INDEPENDENT TEST"
    )
else:
    reg_verdict = (
        "REGRESSION MODEL NOT STRONG ENOUGH"
    )

print(f"Verdict: {reg_verdict}")
print("INDEPENDENT PERIOD NOT USED")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

reg_fold_df.to_csv(
    "spy_sentinel_regression_cv.csv",
    index=False
)

pd.DataFrame([{
    "mean_correlation": round(mean_corr, 5),
    "worst_correlation": round(worst_corr, 5),
    "mean_mae_pct": round(mean_mae * 100, 5),
    "signal_threshold_pct": PRED_THRESHOLD * 100,
    "signals": len(reg_signals),
    "signal_accuracy": round(reg_signal_accuracy, 3),
    "avg_directional_return": round(reg_avg_return, 5),
    "median_directional_return": round(reg_median_return, 5),
    "candidate": reg_candidate,
    "verdict": reg_verdict,
}]).to_csv(
    "spy_sentinel_regression_summary.csv",
    index=False
)

print("Regression research saved")

print("\nV5.8 MARKET-REGIME PERFORMANCE MAP")

regime_map = clean.copy()
...
regime_map["ema50_slope5"] = (
    regime_map["ema50"]
    - regime_map["ema50"].shift(5)
).shift(1)

regime_map["trend_strength"] = (
    abs(
        regime_map["ema9"]
        - regime_map["ema21"]
    )
...
regime_map["true_range"] = pd.concat(
    [high_low, high_close, low_close],
    axis=1
).max(axis=1)

regime_map["atr14"] = (
    regime_map["true_range"]
    .rolling(14)
    .mean()
)
...
print("ATR regime feature created")

regime_map["trend_regime"] = np.where(
    (
        (regime_map["ema9"] > regime_map["ema21"])
        & (regime_map["ema50_slope5"] > 0)
    ),
    "UPTREND",
    np.where(
        (
...
atr_median = regime_map["atr_pct"].median()

regime_map["vol_regime"] = np.where(
    regime_map["atr_pct"] >= atr_median,
    "HIGH_VOL",
    "LOW_VOL"
)

print("Trend and volatility regimes assigned")

regime_map["minutes_et"] = (
    regime_map["time_et"].dt.hour * 60
    + regime_map["time_et"].dt.minute
)

regime_map["session"] = np.where(
    regime_map["minutes_et"] < 660,
    "MORNING",
    np.where(
        regime_map["minutes_et"] < 810,
        "MIDDAY",
        "AFTERNOON"
    )
)

print("Session buckets assigned")
...
for trend_name in [
    ...
    for vol_name in [
        ...
        for session_name in [
            ...
            sample = regime_map[
                (regime_map["trend_regime"] == trend_name)
                & (regime_map["vol_regime"] == vol_name)
                & (regime_map["session"] == session_name)
            ].dropna(
                subset=["future_return_60m"]
            ).copy()
...
regime_df["directional_edge"] = (
    abs(
        regime_df["up_rate"] - 50
    )
)

regime_df["preferred_direction"] = np.where(
    regime_df["up_rate"] >= 50,
    "BULLISH",
    "BEARISH"
)

print("\nSTRONGEST DEVELOPMENT REGIMES")

top_regimes = regime_df.sort_values(
    [
        "directional_edge",
        "signals",
    ],
...
print(
    top_regimes.to_string(
        index=False
    )
)

print("\nV5.8 REGIME DISCOVERY GATE")

usable_regimes = regime_df[
    (regime_df["signals"] >= 100)
    & (regime_df["directional_edge"] >= 5)
].copy()

if not usable_regimes.empty:

    best_regime = usable_regimes.sort_values(
        [
            "directional_edge",
            "signals",
        ],
    ...
    print("Candidate regime found:")
    print(
        f"{best_regime['trend']} | "
        f"{best_regime['volatility']} | "
        f"{best_regime['session']}"
    )
    print(
        f"Preferred direction: "
        f"{best_regime['preferred_direction']}"
    )
    print(
        f"Directional edge: "
        f"{best_regime['directional_edge']:.2f}%"
    )
    print(
        f"Signals: "
        f"{int(best_regime['signals'])}"
    )

else:

    best_regime = None
    regime_candidate = False

    print("No adequately sampled regime shows >=5% directional edge")

print("\nV5.8 RESEARCH STATUS")

if regime_candidate:
    regime_verdict = (
        "REGIME CANDIDATE READY FOR INDEPENDENT TEST"
    )
else:
    regime_verdict = (
        "NO REGIME CANDIDATE FOUND"
    )
...
regime_df.to_csv(
    "spy_sentinel_regime_map.csv",
    index=False
)

print("Regime map saved")

print("\nV5.9 INDEPENDENT REGIME VALIDATION")

ind_regime = independent.copy()

ind_regime["future_return_60m_true"] = np.where(
    ind_regime["date_et"]
    == ind_regime["date_et"].shift(-12),
    ind_regime["close"].shift(-12)
    / ind_regime["close"]
    - 1,
    np.nan
)
...
ind_regime["ema50_slope5"] = (
    ind_regime["ema50"]
    - ind_regime["ema50"].shift(5)
).shift(1)

ind_regime["trend_regime"] = np.where(
    (
        (ind_regime["ema9"] > ind_regime["ema21"])
        & (ind_regime["ema50_slope5"] > 0)
    ),
    "UPTREND",
    np.where(
        (
...
print("Independent trend regime calculated")
...
ind_regime["true_range"] = pd.concat(
    [i_high_low, i_high_close, i_low_close],
    axis=1
).max(axis=1)

ind_regime["atr14"] = (
    ind_regime["true_range"]
    .rolling(14)
    .mean()
)
...
# IMPORTANT: use DEVELOPMENT volatility cutoff
ind_regime["vol_regime"] = np.where(
    ind_regime["atr_pct"] >= atr_median,
    "HIGH_VOL",
    "LOW_VOL"
)

print("Independent volatility regime calculated")

ind_regime["minutes_et"] = (
    ind_regime["time_et"].dt.hour * 60
    + ind_regime["time_et"].dt.minute
)

ind_regime["session"] = np.where(
    ind_regime["minutes_et"] < 660,
    "MORNING",
    np.where(
        ind_regime["minutes_et"] < 810,
        "MIDDAY",
        "AFTERNOON"
    )
)

candidate_trend = best_regime["trend"]
candidate_vol = best_regime["volatility"]
candidate_session = best_regime["session"]
candidate_direction = best_regime["preferred_direction"]

print("\nFROZEN DEVELOPMENT REGIME")
print(f"Trend:     {candidate_trend}")
print(f"Volatility:{candidate_vol}")
print(f"Session:   {candidate_session}")
print(f"Direction: {candidate_direction}")

ind_regime_candidate = ind_regime[
    (ind_regime["trend_regime"] == candidate_trend)
    & (ind_regime["vol_regime"] == candidate_vol)
    & (ind_regime["session"] == candidate_session)
].dropna(
    subset=["future_return_60m_true"]
).copy()

print(
    f"Independent regime observations: "
    f"{len(ind_regime_candidate)}"
)

if candidate_direction == "BULLISH":
    regime_wins = (
        ind_regime_candidate["future_return_60m_true"] > 0
    )
    regime_signed_returns = (
        ind_regime_candidate["future_return_60m_true"]
    )
else:
    regime_wins = (
        ind_regime_candidate["future_return_60m_true"] < 0
...
ind_regime_accuracy = (
    regime_wins.mean() * 100
    if len(ind_regime_candidate)
    else 0.0
)

ind_regime_avg_return = (
    regime_signed_returns.mean() * 100
    if len(ind_regime_candidate)
    else 0.0
)

ind_regime_median_return = (
    regime_signed_returns.median() * 100
    if len(ind_regime_candidate)
    else 0.0
)

print("\nINDEPENDENT REGIME RESULTS")

print(f"Accuracy:       {ind_regime_accuracy:.2f}%")
print(f"Observations:   {len(ind_regime_candidate)}")
print(f"Edge:           {ind_regime_accuracy - 50:+.2f}%")
print(f"Average return: {ind_regime_avg_return:+.4f}%")
print(f"Median return:  {ind_regime_median_return:+.4f}%")

development_regime_accuracy = (
    max(
        float(best_regime["up_rate"]),
        100 - float(best_regime["up_rate"])
    )
)

regime_accuracy_change = (
    ind_regime_accuracy
    - development_regime_accuracy
)

regime_independent_pass = (
    len(ind_regime_candidate) >= 100
    and ind_regime_accuracy >= 55
    and ind_regime_avg_return > 0
    and abs(regime_accuracy_change) <= 10
)

print("\nV5.9 INDEPENDENT REGIME VERDICT")

print(
    f"Development accuracy: "
    f"{development_regime_accuracy:.2f}%"
)

print(
    f"Independent accuracy: "
    f"{ind_regime_accuracy:.2f}%"
)

print(
    f"Change: "
    f"{regime_accuracy_change:+.2f}%"
)

if regime_independent_pass:
    regime_ind_verdict = (
        "PASSES INDEPENDENT REGIME VALIDATION"
    )
else:
    regime_ind_verdict = (
        "DOES NOT PASS INDEPENDENT REGIME VALIDATION"
    )

print(f"Verdict: {regime_ind_verdict}")
print("REGIME RULES WERE FROZEN BEFORE THIS TEST")
print("PAPER TRADING: LOCKED")
print("LIVE TRADING: LOCKED")

pd.DataFrame([{
    "trend": candidate_trend,
    "volatility": candidate_vol,
    "session": candidate_session,
    "direction": candidate_direction,
    "development_accuracy": round(
        development_regime_accuracy, 3
    ),
    "independent_observations": len(
        ind_regime_candidate
    ),
    "independent_accuracy": round(
        ind_regime_accuracy, 3
    ),
    "accuracy_change": round(
        regime_accuracy_change, 3
    ),
    "avg_directional_return": round(
        ind_regime_avg_return, 5
    ),
    "median_directional_return": round(
        ind_regime_median_return, 5
    ),
    "passed": regime_independent_pass,
    "verdict": regime_ind_verdict,
}]).to_csv(
    "spy_sentinel_regime_independent.csv",
    index=False
)

print("Independent regime validation saved")

print("\nV6.0 RESEARCH LEADERBOARD")
...
print(f"Research files expected: {len(research_files)}")
...
print(f"Research strategies loaded: {len(leaderboard)}")

print("\nINDEPENDENT RESEARCH LEADERBOARD")

leaderboard["accuracy"] = pd.to_numeric(
    leaderboard["accuracy"],
    errors="coerce"
)

leaderboard["avg_return_pct"] = pd.to_numeric(
    leaderboard["avg_return_pct"],
    errors="coerce"
)

leaderboard["signals"] = pd.to_numeric(
    leaderboard["signals"],
    errors="coerce"
)
...
for _, row in leaderboard.iterrows():
    ...
    for cost in COST_SCENARIOS_PCT:
        ...
        cost_rows.append({
            ...
            "positive_after_cost": net_return > 0,
        })

cost_df = pd.DataFrame(cost_rows)

print("Cost scenarios calculated")

print("\nNET RETURN AT 0.05% COST")

cost_005 = cost_df[
    cost_df["assumed_cost_pct"] == 0.05
].copy()

cost_005 = cost_005.sort_values(
    "net_return_pct",
    ascending=False
)
...
print("\nRESEARCH SURVIVOR CHECK")

survivors = leaderboard[
    (leaderboard["accuracy"] >= 55)
    & (leaderboard["signals"] >= 100)
    & (leaderboard["avg_return_pct"] > 0.05)
    & (leaderboard["passed"] == True)
].copy()

if survivors.empty:
    survivor_status = "NO VALIDATED STRATEGY SURVIVES"
else:
    survivor_status = "AT LEAST ONE STRATEGY SURVIVES"

print(f"Status: {survivor_status}")
...
print("\nV6.0 HACKATHON RESEARCH STATUS")

if survivors.empty:

    hackathon_research_status = (
        "RESEARCH ENGINE WORKS; PROFITABLE EDGE NOT YET PROVEN"
    )

else:

    hackathon_research_status = (
        "VALIDATED CANDIDATE EXISTS — REQUIRES FURTHER TESTING"
    )

print(hackathon_research_status)

print("\nKEY SAFETY MESSAGE")
print("- Weak strategies were rejected")
print("- Independent validation was enforced")
print("- Testing errors were corrected when discovered")
print("- Transaction costs are now included in evaluation")
print("- Paper trading remains locked")
print("- Live trading remains locked")

leaderboard.to_csv(
    "spy_sentinel_research_leaderboard.csv",
    index=False
)

cost_df.to_csv(
    "spy_sentinel_cost_sensitivity.csv",
    index=False
)

pd.DataFrame([{
    "strategies_tested": len(leaderboard),
    "validated_survivors": len(survivors),
    "status": hackathon_research_status,
    "paper_trading_locked": True,
    "live_trading_locked": True,
}]).to_csv(
    "spy_sentinel_hackathon_research_status.csv",
    index=False
)

print("Research leaderboard and hackathon status saved")
...
print("\nV6.1 HACKATHON CHECKPOINT")

checkpoint = {
    "project": "SPY Sentinel AI",
    "status": hackathon_research_status,
    "strategies_tested": int(len(leaderboard)),
    "validated_survivors": int(len(survivors)),
    "paper_trading_locked": True,
    "live_trading_locked": True,
    "independent_validation": True,
    "transaction_cost_testing": True,
    "audit_logging": True,
...
with open(
    "spy_sentinel_hackathon_checkpoint.json",
    "w"
) as f:
    json.dump(
        checkpoint,
...
print("Checkpoint JSON saved")

report_lines = [
    "# SPY Sentinel AI — Hackathon Research Checkpoint",
    "",
    "## Current Status",
    f"- {hackathon_research_status}",
    f"- Strategies tested: {len(leaderboard)}",
    f"- Validated survivors: {len(survivors)}",
    "",
    "## Working System Components",
    "- Alpaca paper-account connection",
    "- SPY market-data ingestion",
    "- Technical feature engine",
    "- SPY options-chain lookup",
    ...
    "",
    "## Research Conclusion",
    "- Multiple strategies and ML models were tested.",
    "- Weak or unstable strategies were rejected.",
    "- No profitable edge has yet met the validation standard.",
    "- Further research is required before paper execution.",
]

with open(
    "SPY_SENTINEL_HACKATHON_CHECKPOINT.md",
    "w"
) as f:
    f.write("\n".join(report_lines))

print("Hackathon checkpoint report saved")
...
architecture = [
    ...
    "3. Strategy / ML research",
    "4. Independent validation",
    "5. Transaction-cost check",
    "6. Risk gates",
    "7. Options contract selection",
    "8. Position sizing",
    "9. Paper execution lock",
    "10. Audit / research logs",
]

for item in architecture:
    print(item)

print("\nTODAY'S RESEARCH PRINCIPLES")

principles = [
    "No rule changes after seeing independent results",
    "No strategy promoted without adequate sample size",
    "No reliance on raw accuracy alone",
    "Transaction costs included",
    "Testing errors corrected when discovered",
    "No forced trades",
    "Paper/live execution remains locked",
]
...
pd.DataFrame([{
    "checkpoint_time_utc": checkpoint_time,
    "strategies_tested": len(leaderboard),
    "validated_survivors": len(survivors),
    "research_status": hackathon_research_status,
    "paper_locked": True,
    "live_locked": True,
}]).to_csv(
    "spy_sentinel_checkpoint_history.csv",
    mode="a",
    header=not Path(
        "spy_sentinel_checkpoint_history.csv"
    ).exists(),
